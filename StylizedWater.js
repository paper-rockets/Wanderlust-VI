...
export function createProceduralEdgeRampTexture(width = 256) {
    ...
    // Stylized foam ramp: crisp solid white for cartoon foam boundaries
    const grad = ctx.createLinearGradient(0, 0, width, 0);
    grad.addColorStop(0.0, '#ffffff');
    grad.addColorStop(0.65, '#ffffff');
    grad.addColorStop(0.8, '#ffffff');
    grad.addColorStop(0.9, '#c8f0ff');
...
void main() {
    ...
    // Stylized 3D rolling waves across the water surface
    float wave1 = sin(worldPos.x * 0.28 + worldPos.z * 0.18 + uTime * 1.5 * windFactor) * 0.35;
    float wave2 = cos(worldPos.x * 0.48 - worldPos.z * 0.32 + uTime * 2.1 * windFactor) * 0.2;
    float wave3 = sin((worldPos.x + worldPos.z) * 0.14 + uTime * 0.9 * windFactor) * 0.25;
    p.y += (wave1 + wave2 + wave3) * (uDisplacementStrength * 2.0 + 0.1);
#endif
...
uniform float uWaveLapping;
uniform float uSurfaceFoamStrength;
...
void main() {
    ...
    // --------------------------------------------------------
    // EDGE / CONTACT FOAM (SCALLOPED & LAPPING WAVES)
    // --------------------------------------------------------

    // Smooth rhythmic wave lapping against shores and objects
    float lapWave =
        (sin(vWorldPosition.x * 0.75 + uTime * uEdgeSpeed * 2.5) * 0.65 +
         cos(vWorldPosition.z * 0.9 + uTime * uEdgeSpeed * 1.8) * 0.35)
        * uWaveLapping;

    float effectiveWaterDepth =
        max(waterDepth - lapWave, 0.0);
    ...
    // Crisp stylized cartoon foam band
    float foamSample =
        texture2D(
            tEdgeRamp,
            vec2(
                clamp(
                    edgeNoise * (1.0 - edgeBlend * 0.7),
                    0.0,
                    1.0
                ),
                0.5
    ...
    // Sharp clean cartoon foam outline
    float edgeMask =
        smoothstep(0.16, 0.28, foamSample * edgeBlend);

    // Floating detached foam dots / bubbles near shoreline (Image 1)
    float bubbleNoise = texture2D(tEdgeNoise, vWorldPosition.xz * 0.9 + uTime * 0.03).r;
    float bubbles =
        step(0.74, bubbleNoise) *
        smoothstep(uEdgeThickness * 1.6, uEdgeThickness * 0.3, effectiveWaterDepth);

    edgeMask =
        clamp(edgeMask + bubbles * 0.9, 0.0, 1.0);
    ...
    // Floating surface foam flecks drifting across open water (Image 2)
    vec2 surfUV =
        vWorldPosition.xz * 0.09 +
        -windXZ * uTime * 0.04;

    float surfNoise =
        texture2D(tEdgeNoise, surfUV).r;

    float surfaceFoam =
        step(0.82, surfNoise) *
        uSurfaceFoamStrength;
    ...
    // Vibrant anime shallow water tint (as in Image 1 and Image 2)
    float shallowFactor = 1.0 - clamp(finalWaterDepth / 1.6, 0.0, 1.0);
    color = mix(color, mix(color, uShallowColor, 0.72), shallowFactor);
        ...
        this.material =
            new THREE.ShaderMaterial({
                ...
                uniforms: {
                    ...
                    uAbsorptionColor: {
                        value: new THREE.Color(0.85, 0.25, 0.0)
                    },

                    uShallowColor: {
                        value: new THREE.Color(0.25, 0.94, 0.88)
                    },
                    ...
                    uFresnelColor: {
                        value: new THREE.Color(0.2, 0.88, 0.98)
                    },
                    ...
                    uEdgeNoiseScale: {
                        value: new THREE.Vector2(0.35, 0.35)
                    },
