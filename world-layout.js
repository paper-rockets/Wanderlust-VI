...
export const HILL_HEIGHT = 1.35; // increased terrain heights for dramatic vertical presence
const INV_WORLD = 1.0 / WORLD_SCALE;
const INV_HILL = 1.0 / HILL_SCALE;
const DESIGN_WORLD_BOUNDS = 58000;
...
export class WorldLayout {
    ...
    generate(seed = this.seed) {
        ...
        const oceanGap = 2000; // Minimum open ocean gap between major islands

        // 1. Place Major Biome Islands with Rejection Sampling (Two Concentric Rings)
        for (let i = 0; i < requiredBiomes.length; i++) {
            const biomeDef = requiredBiomes[i];
            let placed = false;
            let bestCandidate = null;

            // Two concentric rings: alternating outer and inner ring for balanced coverage
            const isOuter = (i % 2 === 0);
            const baseAngle = (i / requiredBiomes.length) * Math.PI * 2 + this.rng.range(-0.15, 0.15);
            const ringDist = isOuter ? this.rng.range(26000, 38000) : this.rng.range(13000, 23000);

            for (let attempt = 0; attempt < 200; attempt++) {
                const angle = baseAngle + this.rng.range(-0.35, 0.35);
                const dist = attempt === 0 ? ringDist : (isOuter ? this.rng.range(24000, 42000) : this.rng.range(11000, 26000));
                const cx = Math.cos(angle) * dist + this.rng.range(-800, 800);
                const cz = Math.sin(angle) * dist + this.rng.range(-800, 800);

                // Island size and elliptical distortion
                const isLarge = biomeDef.primaryClass === 'large';
                const baseRad = isLarge ? this.rng.range(5800, 7500) : this.rng.range(4200, 5600);
                const aspect = this.rng.range(0.8, 1.25);
    ...
    sampleIslandMask(island, worldX, worldZ, snoise) {
        ...
        const warp1 = snoise(dX * 0.00045 + seedOff, dZ * 0.00045 + seedOff) * 0.20;
        const warp2 = snoise(dX * 0.0010 - seedOff, dZ * 0.0010 + seedOff) * 0.08;
        const coastNoise = snoise(dX * 0.0018 + seedOff * 2, dZ * 0.0018 - seedOff * 2) * 0.03;

        const effectiveDist = normDist + warp1 + warp2 + coastNoise;

        if (effectiveDist >= 1.0) return 0.0; // Ocean
        if (effectiveDist <= 0.20) return 1.0; // Core interior

        // Smooth coastline slope transition from 0.0 at shore to 1.0 interior
        return smoothstep(1.0, 0.20, effectiveDist);
    }
    ...
    getHeight(worldX, worldZ, snoise) {
        ...
        let maxIslandMask = 0;
        let dominantBiomeId = null;


        // Base ocean floor with gentle deep submarine relief
        const oceanRelief = snoise(worldX * INV_WORLD * 0.0006, worldZ * INV_WORLD * 0.0006) * 2.5 - 5.5;
        ...
        for (let i = 0; i < candidates.length; i++) {
            ...
            if (mask > 0.0001) {
                ...
                const isMountain = isl.biome.id === 'misty_mountains' || isl.biome.id === 'misty_mountains_2';
                let totalIslandElevation;
                if (isMountain) {
                    // Exact mountain elevation from Mountains (no coordinate compression, no dome smoothing)
                    const rawH = isl.biome.module.getHeight(worldX, worldZ, snoise);
                    const edgeRamp = smoothstep(0.0, 0.15, mask);
                    totalIslandElevation = oceanRelief + (rawH - oceanRelief) * edgeRamp;
                } else {
                    const rawH = isl.biome.module.getHeight(lx * INV_HILL, lz * INV_HILL, snoise) * HILL_HEIGHT;

                    // Natural island elevation profile:
                    // Shoreline meets water smoothly at y = 2.4 - 3.0m with gentle beach slopes
                    // Shore transition: ocean floor rises softly into beach
                    const shoreRamp = smoothstep(0.0, 0.38, mask);
                    const beachBase = oceanRelief + (3.0 - oceanRelief) * shoreRamp;

                    // Mountains and hills rise softly inland
                    const hillRamp = smoothstep(0.25, 0.95, mask);
                    totalIslandElevation = beachBase + (rawH * isl.heightMultiplier) * hillRamp;
                }

                // Smoothly blend overlapping island heights (eliminates sharp seam creases)
                if (isMountain) {
                    if (maxElevation === -999 || totalIslandElevation > maxElevation) {
                    ...
                    if (maxElevation === -999) {
                        ...
                        const diff = totalIslandElevation - maxElevation;
                        const blendK = 3.5;
                        const h = Math.max(0, Math.min(1, 0.5 + 0.5 * diff / blendK));
                        maxElevation = (1.0 - h) * maxElevation + h * totalIslandElevation + blendK * h * (1.0 - h);
                    }
                }

                if (mask > maxIslandMask) {
                    maxIslandMask = mask;
                    dominantBiomeId = isl.biome.id;
                }
            }
        }
        ...
        return this.applyLandforms(maxElevation, worldX, worldZ, maxIslandMask, snoise, dominantBiomeId);
    }
    ...
    applyLandforms(h, x, z, landMask, snoise, biomeId = null) {
        if (biomeId === 'misty_mountains' || biomeId === 'misty_mountains_2') {
            return h;
        }
        const inland = smoothstep(0.18, 0.50, landMask);

        // Coastal sea cliffs on select outer island headlands (rises dramatically above the ocean)
        const seaCliffZone = smoothstep(0.25, 0.62, snoise(x * 0.0011 + 888.0, z * 0.0011 - 888.0));
        const headland = smoothstep(0.14, 0.36, landMask) * (1.0 - smoothstep(0.40, 0.68, landMask));
        if (seaCliffZone > 0 && headland > 0) {
            h += seaCliffZone * headland * 24.0;
        }

        if (inland <= 0) return h;

        // Rich undulating rolling hills across the island interior
        const h1 = snoise(x * 0.0014 + 120.0, z * 0.0014 - 220.0);
        const h2 = snoise(x * 0.0028 - 360.0, z * 0.0028 + 160.0);
        const hillWaves = (Math.max(0, h1 + 0.15) * 18.0 + Math.max(0, h2) * 9.0) * inland;
        h += hillWaves;

        // Dramatic cliff escarpments & plateau bluffs (smooth steep cliff walls)
        const cliffFault = snoise(x * 0.00095 - 600.0, z * 0.00095 + 750.0);
        const cliffWarp = snoise(x * 0.0022 + 180.0, z * 0.0022 - 180.0) * 0.12;
        const cliffVal = cliffFault + cliffWarp;
        const cliffRamp = smoothstep(0.06, 0.38, cliffVal);
        const cliffFade = inland * smoothstep(6.0, 60.0, h);
        h += cliffRamp * 28.0 * cliffFade;

        // Rounded ridge crests on higher mountains
        const rq = snoise(x * 0.0018 + 311.0, z * 0.0018 - 127.0);
        const rn = 1.0 - rq * rq;
        h += rn * 10.0 * inland * smoothstep(12.0, 45.0, h) * (1.0 - smoothstep(65.0, 120.0, h));

        // Soft rock contours
        const contourZone = smoothstep(0.25, 0.60, snoise(x * 0.0009 - 900.0, z * 0.0009 + 400.0)) * inland * 0.30;
        const contourFade = 1.0 - smoothstep(30.0, 60.0, h);
        if (contourZone > 0 && h > 10.0 && contourFade > 0) {
            const wave = Math.sin((h - 10.0) * 0.30);
            h += wave * 2.5 * contourZone * contourFade;
        }

        // Winding rivers (soft wide banks, no steep cuts)
        const wx = x + snoise(x * 0.0016 + 77.0, z * 0.0016 - 33.0) * 60.0;
        const wz = z + snoise(x * 0.0016 - 51.0, z * 0.0016 + 19.0) * 60.0;
        const rv = Math.abs(snoise(wx * 0.0008 + 500.0, wz * 0.0008 + 500.0));
        const river = (1.0 - smoothstep(0.01, 0.25, rv)) * inland * (1.0 - smoothstep(10.0, 28.0, h));

        // Soft ponds in low ground
        const pn = snoise(x * 0.0035 + 1300.0, z * 0.0035 - 700.0);
        const pond = smoothstep(0.50, 0.88, pn) * inland * (1.0 - smoothstep(10.0, 24.0, h));

        const carve = Math.max(river, pond);
        if (carve > 0) {
            const smoothCarve = carve * carve * (3.0 - 2.0 * carve); // smooth cubic ease
            h = Math.min(h, h + (1.2 - h) * smoothCarve * 0.85);
        }
        return h;
    }
    ...
    getColor(h, worldX, worldZ, snoise, targetColor, blendColor1, blendColor2) {
        ...
        if (bestIsl && maxMask > 0.01) {
            const isMtn1 = bestIsl.biome.id === 'misty_mountains' || bestIsl.biome.id === 'misty_mountains_2';
            const bx1 = isMtn1 ? worldX : worldX * INV_HILL;
            const bz1 = isMtn1 ? worldZ : worldZ * INV_HILL;
            ...
            if (secondIsl && secondMask > 0.15) {
                ...
                const isMtn2 = secondIsl.biome.id === 'misty_mountains' || secondIsl.biome.id === 'misty_mountains_2';
                const bx2 = isMtn2 ? worldX : worldX * INV_HILL;
                const bz2 = isMtn2 ? worldZ : worldZ * INV_HILL;
        ...
        for (let i = 0; i < testSeeds.length; i++) {
            ...
            for (let a = 0; a < majors.length; a++) {
                for (let b = a + 1; b < majors.length; b++) {
                    ...
                    if (dist < (majors[a].maxRadius + majors[b].maxRadius + 500 * WORLD_SCALE)) {
                        majorOverlap = true;
                    }
                }
            }
            ...
            for (const isl of world.islands) {
                if (Math.abs(isl.centerX) + isl.maxRadius > world.worldBounds + 2000 * WORLD_SCALE ||
                    Math.abs(isl.centerZ) + isl.maxRadius > world.worldBounds + 2000 * WORLD_SCALE) {
                    outOfBounds = true;
                }
            }
